/**
 * BarScanEmptiesPage — Mode C (Bartender CONSUMED).
 *
 * The page Marco the Bartender uses after the event ends (or during,
 * for high-throughput bars) to scan each empty bottle before disposing.
 * The system records a CONSUMED scan against his bar and the live
 * event; server-side warehouse_inventory drops its on-hand count.
 *
 * Difference between Mode B (arrivals) and Mode C (empties):
 *   Mode B count − Mode C count = expected leftover stock at the bar
 *   The reconciliation report (Phase 6.11) compares both against
 *   warehouse_allocations.dispatched_qty + actual POS sales to surface
 *   shrinkage / over-pour / theft signal per product per bar.
 *
 ...
 *
 * Route: /scan/empties  (wired in Step 6.8).
 * Permission: canScanEmptiesAtBar  (Bartender + Owner per Step 6.5).
 *
...
export function BarScanEmptiesPage() {
  const { user } = useAuth()
      ...
      <section className="mt-6">
        ...
        {history.length === 0 ? (
          <p className="text-sm text-[#A0AEC0] py-6 text-center bg-[#F7FAFC] rounded-lg border border-dashed border-[#E2E8F0]">
            No empties scanned yet. When a bottle is empty, point the camera
            at its barcode before disposing, or type it manually above.
          </p>
        ) : (
