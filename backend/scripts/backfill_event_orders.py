...
Uses 2-minute time chunks to work around Slesh's broken pagination
(100-order-per-call cap with non-working `from` offset).
...
async def main():
    ...
    total_chunks = int((end - start).total_seconds() / 60 / CHUNK_MINUTES)
    print(f"Event:   {event_id}")
    print(f"Window:  {start.isoformat()} -> {end.isoformat()}")
    print(f"Chunks:  {CHUNK_MINUTES}-min ({total_chunks} total)")
    ...
    async with AsyncSessionLocal() as db:
        ...
        seen = inserted = chunks_done = 0
        by_type = {}
        chunk_dur = timedelta(minutes=CHUNK_MINUTES)
        cursor = start

        while cursor < end:
            chunk_end = min(cursor + chunk_dur, end)

            async for order in adapter.list_orders(cursor, chunk_end, order_type=None):
                seen += 1
            ...
            if chunks_done % 30 == 0:
                if not args.dry_run:
                    await db.commit()
                print(f"  chunk {chunks_done}/{total_chunks} | {cursor.strftime('%H:%M')} UTC | seen={seen} inserted={inserted}")
