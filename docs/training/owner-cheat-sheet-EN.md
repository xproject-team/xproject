...
| **Anomaly — Demand spike** | One product is selling 2-3× faster than normal | You only (silent — manager doesn't see this) |
| **Anomaly — Recipe deviation** | A bar is using more ingredient than the recipe says | You only (silent — manager doesn't see this) |

The "silent" anomaly classes are intentional. They give you visibility on potential staff issues (over-pouring, theft, miscounting) without making the manager defensive or warning the staff member. **Acknowledge an anomaly alert** to trigger a neutral "please do a routine count" message to the bar's chat — the manager investigates without knowing you suspect something.
...
The first command proves alerts still fire. The second tells you which subsystem (Postgres, Redis, MinIO) is down.
